...
class Grid implements Iterable<Cell> {
    ...
    // constructor
    public Grid() {
        CellIterator gridCells = new CellIterator(cells);
        while (gridCells.hasNext()) {
            int i = gridCells.outer;
            int j = gridCells.inner;
    ...
    public List<Cell> cellsInRange(char c1, int r1, char c2, int r2) { //Depature
    ...
    public Optional<Cell> cellAtPoint(Point p) {
        Iterator<Cell> temp = this.iterator(); //Depature
        while (temp.hasNext()) {
            // for (Cell c : this) {
            Cell c = temp.next();
            if (c.contains(p)) {
    ...
    public void paintOverlay(Graphics g, List<Cell> cells, Color colour) {
        g.setColor(colour);
        CellIterator temp = new CellIterator(cells);
        while (temp.hasNext()) {
            Cell c = temp.next();
            // for (Cell c : cells) {
